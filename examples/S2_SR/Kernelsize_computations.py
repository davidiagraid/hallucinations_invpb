...
from accuracy_bounds.inverseproblems.feasible_sets_dataloader import feasible_appartenance_additive_noise_dataloader_cuda 
...
if __name__ == '__main__':
    ...
    if not light_load:
        ...
        if Test:
            ...
            random_indexes = [2363, 79383, 58493]
            data_idx = [forwarded_target_loader_light.dataset[x] for x in random_indexes]
            data = forwarded_target_loader_light.collate_fn(data_idx)
    ...
    sparse.save_npz(os.path.join(results_fp, f'feas_app_LL_PS{patchsize_X}_NL{noise_level}'), feas_app_save)
    ...
    sparse.save_npz(os.path.join(results_fp, f'distsXX_LL_PS{patchsize_X}_NL{noise_level}'), distsXX)
