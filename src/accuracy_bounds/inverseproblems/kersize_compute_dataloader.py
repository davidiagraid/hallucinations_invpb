...
def compute_dists_on_sparse(dataloader, sparse_matrix, p_norm, batchsize):
    '''
    general function to compute the distance between data points with index being nonzero elements of a square matrix
    '''
    # First get the non zero indexes of common_feasible as a n,2 shaped array
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    rows, cols = sparse_matrix.nonzero()
    index_array = np.vstack((rows, cols)).T 
    ...
    for i_batch in range(n_batches):
        t = time()-t0
        print(f"Target Batch: [{i_batch+1} / {n_batches}],  time = {int(t)} seconds", end="\r")
        ...
        data1 = [dataloader.dataset[batch_indices[i,0]] for i in range(len_batch)]
        data2 = [dataloader.dataset[batch_indices[i,1]] for i in range(len_batch)]

        data1 = dataloader.collate_fn(data1)
        data2 = dataloader.collate_fn(data2)
        
        X1 = data1.reshape(data1.shape[0], -1)
        X2 = data2.reshape(data2.shape[0], -1)
        ...
        distances_batch = torch.norm(X1-X2, p = p_norm, dim = -1)
    ...
    distsPAIR = torch.sparse_coo_tensor(indices=index_array.transpose(), values = all_distances, size=(n_dataset, n_dataset))
    
    return distsPAIR

def input_distances_candidates(input_data, feasible_appartenance_candidates, p_Y, batchsize):
    '''
    Computes the distances between 2 inputs that correspond to the same Fy
    !!! It assumes that the rows and columns for feasible_appartenance (square matrix) are from the same dataset of inputs
    according to a candidates feasible_appartenance matrix calculated using a bigger epsilon than a real noise model 
    '''

    n_inputs = feasible_appartenance_candidates.shape[1]
    for i in range(min(n_inputs),feasible_appartenance_candidates.shape[0]):
        feasible_appartenance_candidates[i,i] = 0
    return compute_dists_on_sparse(input_data, feasible_appartenance_candidates, p_norm=p_Y, batchsize=batchsize)
...
def target_distances_cuda_V2(target_data, feasible_appartenance, p_X, batchsize):
    ...
    common_feasible = feasible_appartenance@(feasible_appartenance.transpose())

    return compute_dists_on_sparse(target_data, common_feasible, p_X, batchsize)

    '''# First get the non zero indexes of common_feasible as a n,2 shaped array
    rows, cols = common_feasible.nonzero()
    index_array = np.vstack((rows, cols)).T 
...
def kersize_new_inputs(distsXX, feasible_appartenance_new):
    ...
    n,p = feasible_appartenance_new.shape
    return list(Parallel(n_jobs=-1, backend='threading')(delayed(get_info)(y_idx, feasible_appartenance_new, distsXX) for y_idx in tqdm(range(p))))
