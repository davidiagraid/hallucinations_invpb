...
def distsYY_withineps_dataloader_cuda(input_data, forwarded_target, p_Y, epsilon):
    """
    Computes the ditsYY for |y-y'| < epsilon  
    ...
    distsYY = torch.sparse_coo_tensor(indices=torch.empty((2, 0), dtype=torch.long),  # no nonzero entries yet
    ...
    for target_batch_id, target_batch in enumerate(forwarded_target):
        for input_batch_id, input_batch in enumerate(input_data):
            t = time()-t0
            print(f"Target Batch: [{target_batch_id+1} / {len(forwarded_target)}],     Input Batch: [{input_batch_id+1} / {len(input_data)}] time = {int(t)} seconds                   ", end="\r")
            
            distsYY_small = distsYY_withineps_batch_cuda(input_batch, target_batch, p_Y, epsilon)
            distsYY_small = distsYY_small.to_sparse_csr()
            ...
            distsYY_small = offset_csr_block(distsYY_small.to_sparse_csr(), i0, j0, (n_target, n_input))

            distsYY = insert_no_overlap_keep_A(distsYY, distsYY_small.detach().cpu())

    distsYY = distsYY.to_sparse_coo()
    return distsYY

    
def distsYY_withineps_batch_cuda(input_data, forwarded_target, p_Y, epsilon):
    ...
    distsYY = torch.cdist(forwarded_flat, input_data_flat, p = p_Y)
    feasible_appartenance = distsYY<epsilon
    distsYY *= feasible_appartenance
...
def feasible_appartenance_new_input_cuda(new_inputs, forwarded_target, p_Y, epsilon):
    '''
    Given a batch of new inputs, compute the feasible sets associated to each input in the batch
    '''
    assert isinstance(forwarded_target, DataLoader) , 'Data input is only supported as Dataloader'
    assert isinstance(forwarded_target.sampler, SequentialSampler),  'Dataloaders with Random samplers are not supported'
    
    n_newinput = new_inputs.shape[0]
    n_target = len(forwarded_target.dataset)

    feasible_appartenance = torch.sparse_coo_tensor(indices=torch.empty((2, 0), dtype=torch.long),  # no nonzero entries yet
                            values=torch.tensor([], dtype=torch.float32),
                            size=(n_target, n_newinput))
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # For each batch in the forwarded target dataset compute the feasible appartenance
    for forwarded_batch_id, forwarded_batch in enumerate(forwarded_target):
        print(f"Forwarded target Target Batch: [{forwarded_batch_id+1} / {len(forwarded_target)}]                    ", end="\r")
        feasible_small = feasible_appartenance_additive_noise_batch_cuda(new_inputs, forwarded_batch, p_Y, epsilon)
        feasible_small = feasible_small.to_sparse_csr()

        batch_size_forwarded = forwarded_batch.shape[0]
        batch_size_input = new_inputs.shape[0]

        idx_imin = batch_size_forwarded * forwarded_batch_id
        idx_imax = min(idx_imin + batch_size_forwarded, n_target)

        idx_jmin = 0
        idx_jmax = batch_size_input

        i0, i1 = idx_imin, idx_imax
        j0, j1 = idx_jmin, idx_jmax

        feasible_small = offset_csr_block(feasible_small.to_sparse_csr(), i0, j0, (n_target, n_newinput))
...
def feasible_appartenance_additive_noise_dataloader_cuda(input_data, forwarded_target, p_Y, epsilon):
    ...
    for target_batch_id, target_batch in enumerate(forwarded_target):
        ...
        for input_batch_id, input_batch in enumerate(input_data):
            ...
            feasible_small = offset_csr_block(feasible_small.to_sparse_csr(), i0, j0, (n_target, n_input))
...
def feasible_appartenance_additive_noise_short_dataloader_cuda(input_data, forwarded_target, p_Y, epsilon, batchsize):
    """
